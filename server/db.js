...
if (gachaCount === 0) {
  ...
  // 文房具ガチャ（normal）
  insertPrize.run(1, '単語帳',            '英単語を覚えようね',                 '📝', 'N',  30, -1, -1);
  insertPrize.run(1, 'おもしろ消しゴム',  '消しゴムを一つ選んでね',             '🍡', 'N',  42, -1, -1);
  insertPrize.run(1, 'ヘアクリップ',      '前髪が邪魔にならない！',             '📎', 'R',  22, 20, 20);
  insertPrize.run(1, '靴下',              '可愛い靴下がたくさん！！',           '🧦', 'SR',  5, 10, 10);
  insertPrize.run(1, 'シャーペン券',      '好きなシャーペンを買ってもらえるよ', '🖊', 'SSR', 1, 2,  2);

  // 食べ物ガチャ（premium）
  insertPrize.run(2, 'ブタメン',          'おいしいねー',                       '🍜', 'N',  25, 50, 50);
  insertPrize.run(2, 'アイスバー',        '好きなアイスを選んでね！',           '🍦', 'R',  36, -1, -1);
  insertPrize.run(2, 'ラムネ',            '夏の飲み物といえば！',               '🍹', 'R',  22, 15, 15);
  insertPrize.run(2, 'ハーゲンダッツ',    'みんな大好き',                       '🍨', 'SR', 15, 18, 18);
  insertPrize.run(2, 'ミスドドーナツ二つ券','ドーナツと交換できるよ！',         '🍩', 'SSR', 2, 3,  3);
