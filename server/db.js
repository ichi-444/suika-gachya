...
if (!hasColumn('gachas', 'cost')) {
  ...
  db.prepare("UPDATE gachas SET cost = 5  WHERE key = 'normal'").run();
  db.prepare("UPDATE gachas SET cost = 15 WHERE key = 'premium'").run();
  db.prepare("UPDATE gachas SET cost = 10 WHERE key = 'daily'").run();
}
...
if (gachaCount === 0) {
  const insertGacha = db.prepare(`
    ...
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `);
  insertGacha.run('normal',  'ノーマルガチャ',   '気軽に回せる定番ガチャ',     '#3ad0e0', '🍉', 5,  1);
  insertGacha.run('premium', 'プレミアムガチャ', 'ちょっといい景品が当たる！', '#ffb400', '🌟', 15, 2);
  insertGacha.run('daily',   '日替わりガチャ',   '本日限定！特別ラインナップ', '#ff5a6e', '🍧', 10, 3);
  ...
  // Normal
  insertPrize.run(1, 'はずれ',        'また挑戦してね',     '😢', 'N',  50, -1, -1);
  insertPrize.run(1, 'うまい棒',      'みんな大好き',       '🍡', 'N',  30, 200, 200);
  insertPrize.run(1, 'ジュース',      '冷たい飲み物',       '🥤', 'R',  15, 50, 50);
  insertPrize.run(1, 'お菓子セット',  '色々詰め合わせ',     '🍬', 'SR', 5,  20, 20);

  // Premium
  insertPrize.run(2, 'プレミアム菓子','高級スナック',       '🍪', 'R',  40, 50, 50);
  insertPrize.run(2, '文房具セット',  '勉強で活躍',         '✏️', 'R',  30, 30, 30);
  insertPrize.run(2, 'カフェ券',      '500円分',            '☕', 'SR', 20, 10, 10);
  insertPrize.run(2, '図書カード',    '1000円分',           '📚', 'SSR',10, 5, 5);

  // Daily
  insertPrize.run(3, '本日のスタンプ','日替わり',           '🎯', 'R',  50, -1, -1);
  insertPrize.run(3, 'プチギフト',    '日替わりプレゼント', '🎀', 'SR', 30, 20, 20);
  insertPrize.run(3, '本日の大当たり','本日限定ラッキー',   '💎', 'SSR',20, 5, 5);
}
